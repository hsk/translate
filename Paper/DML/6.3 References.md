...
- 6.3 リファレンス
	...
	このセクションでは、我々はλΠに追加、Σは別の副作用のプログラミング機能を撫で：参照を。

	私たちは、最初の参照型（τ）REFを形成するために、型τを取る単項型コンストラクタrefを紹介する。
	私たちは、型コンストラクタrefを扱うための以下の静的なサブタイプのルールが必要になります。
	...
	これ、その型の引数の非変異体は、REFを考慮する。
	我々はまた、次の定義済みの関数refの存在を前提とし、！割り当てられたC-タイプを持つ=（中置）：（接頭辞）と：

		ref : ∀α.(α) ⇒ (α) ref! : ∀α.((α)ref) ⇒ α := : ∀α.((α)ref, α) ⇒
	...
	私たちは、リファレンスL1定数、L2の無限集合の上範囲にLを使用しています。 。 。、単に自然数として表現されていると仮定している一つの。
	私たちは、Mを使用して、などの有限のマッピングを以下に定義され、それぞれ店舗やストアタイプ、のためにμ：
	...
	私たちは型変数は、ストア型には存在することを可能にないことに注意してください。
	言い換えれば、各L∈DOM（μ）、μ（l）は自由型変数を含むことができる。
	...
	私たちは、店舗M0は別のストアMを拡張すると言うなら、M、すべてのL∈DOM（M）⊆DOM（M0）用（L）= M0（L）。

	同様に、我々は、店舗型μ0が別のストア·タイプμを拡張すると言うなら、すべてのL∈DOMのためのμ（L）=μ0（L）（μ）⊆DOM（μ0）。
	...
	- 定義6.6（ステートフルリダクション）
		...
		- E1、→EV E2が保持している場合、我々は→EV/ ST（M、E2）、（M、E1）がある。

		- もし、E1= E [REF（V）]を選び、私たちは持っている（M、E1）、→EV/ ST（M[L7→V]、E [HI]）任意の参照定数L6∈DOM用（M） 。
			...
			だから、非決定性は、この場合に関与していると思われる。

			非決定性のこの形態を排除することができる私たちは（M、E）とを同一視した場合（M0、E0）一つのが適切に参照定数の名前を変更することで、他から得ることができるときはいつでも。それは、この論文では必要ありませんこのような名前の変更アルゴリズムの正確な定義が省略されている。

		- If e1 = E[!l] and M(l) = v, then we have (M, e1) ,→ev/st (M, E[v]).

		- E1= E[！L]とM（L）は= vであれば、我々は→EV/ ST、（M、E1）がある（、M E[V]）。
	...
	- 定義6.7
		...
		多型、例外およびリファレンスで拡張λpatでつの式e1とe2を考えると、我々は、E1≤dynE2は、どの店舗M1といずれかの場合には保持していると言う
		...
		特に、我々は容易に補題2.14はまだ（縮小関係を生成するように、→gはまだ同じように定義される）を保持していることを確認できます。

		大部分は（ハーパー、1994）で提示一つのに基づいているように入力参照、へのアプローチを次のように私たちは現在、概要を説明します。
		...
		eは| |≤dynvは、いくつかの値vのために保持している私たちは、あれば式eが値等価であると言う。
		...
		式にルール（TY-Π-イントロ）を適用した場合の例として、私たちは式が値と同等のでなければならないことを確認する必要があります。
		...
		μは次のように店のMが正式に定義されているストアのタイプμを、割り当てることができることを意味するために：店舗Mと店舗タイプμを考えると、私たちは、Mは書く：
	...
	- 定理6.8（件名リダクション）
		...
		M1を想定しますμ1が成り立つと∅。 ∅。 ∅|- μ1E1：σは導出可能。 （M1、E1）、→EV/ ST（M2、E2）を保持している場合は、M2はそのようなことをμ1拡張店タイピングμ2が存在する：μ2は∅保持して、 ∅。 ∅|- μ2E2：σは導出可能。
