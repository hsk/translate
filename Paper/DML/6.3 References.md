...
- 6.3 リファレンス
	...
	我々は、最初の参照型（τ）REFを形成するために、型τを取る単項型コンストラクタrefを紹介する。
	我々は、型コンストラクタrefを扱うための以下の静的なサブタイプのルールが必要になります。
	...
	我々は、リファレンスL1定数、L2の無限集合の上範囲にLを使用しています。 。 。、単に自然数として表現されていると仮定している一つの。
	我々は、Mを使用して、などの有限のマッピングを以下に定義され、それぞれ店舗やストアタイプ、のためにμ：
	...
	我々は型変数は、ストア型には存在することを可能にないことに注意してください。
	...
	我々は、店舗M0は別のストアMを拡張すると言うなら、M、すべてのL∈DOM（M）⊆DOM（M0）用（L）= M0（L）。
	...
	- 定義6.6（ステートフルリダクション）
		...
		- E1、→EV E2が成り立つ場合、我々は→EV/ ST（M、E2）、（M、E1）がある。

		- もし、E1= E [REF（V）]を選び、我々は持っている（M、E1）、→EV/ ST（M[L7→V]、E [HI]）任意の参照定数L6∈DOM用（M） 。
			...
			非決定性のこの形態を排除することができる我々は（M、E）とを同一視した場合（M0、E0）一つのが適切に参照定数の名前を変更することで、他から得ることができるときはいつでも。それは、この論文では必要ありませんこのような名前の変更アルゴリズムの正確な定義が省略されている。
