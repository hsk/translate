...
- 6.3 リファレンス
	...
	このセクションでは、我々は `λΠ,Σ pat` 別の副作用のプログラミング機能 - 参照 - を追加します。

	我々は、最初の参照型(τ) ref を形成するために、型τを取る単項型コンストラクタrefを紹介します。
	我々は、型コンストラクタrefを扱うための以下の静的なサブタイプのルールが必要になり：
	...
	ここで、その型の引数の非変異体は、refを考慮しています。
	我々はまた、次の定義済みの関数refの存在を前提として、（前置演算子）！と（中置演算子）:= に割り当てられたc-typeを持ちます：
	...
	我々は、Mとμ、それぞれストアとストア型を使用し、これらは以下に定義されたの有限のマッピングです：
	...
	我々は、ストアM0は別のストアMを拡張すると言うなら、M、すべてのL∈DOM（M）⊆DOM（M0）用（L）= M0（L）。
