...
ここをきれいになおす
		...
		- If e1 = E[l := v] and l ∈ dom(M), then we have (M, e1) ,→ev/st (M0, E[hi]),
			where M0 is a store such that dom(M0) = dom(M) and M0(l) = v and M0(l0) = M(l0) for every l0 in dom(M) that is not l.
...
ここをきれいになおす
		...
		context G, either (M1, G[e2]) ,→∗ev/st (M2, Error) holds for some store M2, or (M1, G[e1]) ,→∗ev/st (M2, ans∗) if and only if (M1, G[e2]) ,→∗ev/st (M2, ans∗), where M2 ranges over stores and ans∗ ranges over the set of observable answers.
		...
		動的なサブタイプ関係≤dtpは（定義4.15）の定義は≤dynの上記の定義に応じて変更することができます。
		...
		タイピングの判断はフォームφの今です。 P〜; Γ| - μのE：σ、および以前のすべての型付け規則が必要に応じて修正されます。
		...
		`|e| ≤dyn v` いくつかの値vで成り立つならば、我々は、式 `e` が値等価であると言います。
		...
		ルール (ty-poly) において、α〜＃μはのためにμ（L）におけるαの空き発生していないことを意味し、任意のα∈α〜、lは、dom(μ) 上の範囲です。
		...
		式にルール(ty-Π-intro)を適用した場合の例として、我々は式が値と同等のでなければならないことを確認する必要があります。

		これは、値の制限の通常の形ML内のインスタンスのために、課される(Wright, 1995)、若干異なっています。

		値の推敲は必ずしも値ではない場合がありますので、軽微な変更が必要とされています。

		例えば、ルール (elab-dn-up) が適用されている場合、これは起こることがあります。

		補題5.2と補題2.14により、我々は値の推敲は、常に価値と同等であることを知っています。
	...
	- 定理6.8（件名リダクション）
		...
		もしも `(M1, e1) ,→ev/st (M2, e2)` が成り立つ場合は、 `M2 : µ2` が成り立ち `∅; ∅; ∅ |- µ2 e2 : σ` が導出可能であるような `µ1` を拡張するストア型 `µ2` が存在します。
	...
	- 定理6.9（進捗状況）
		...
		- eは値Vである、または
		- eで発生した例外raise(v)、または
		- `M`を拡張した`M0`のようないくつかの`M0`と式 `e0`で、 (M, e) ,→ev/st (M0, e0)が成り立つ。

		これらの2つの定理の証明は、本質的に、定理4.11と定理4.12と同様であり、いくつかの関連する詳細も(Harper, 1994)に見出すことができます。
		付録Cで、我々は明らかに値制限のいくつかの関与を実証定理6.8の証明のスケッチを提供します。
