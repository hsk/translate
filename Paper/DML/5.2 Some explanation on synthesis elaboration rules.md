...
- 5.2 合成推敲規則の一部の説明
	...
	ルール（機能一覧アップAPP-2）は、実際にはむしろ複雑なしかし非常に重要である、と我々はそれのためにいくつかの説明を提供。 E1（E2）の型を合成するとき、私たちはまずτ2→フォームτ1のものではないE1の型τを合成すること。例えば、τは普遍的に定量化された型であってもよい。そうである場合、我々は次e2のための型を合成し、次いで、ルール（イーラボアップアプリ2）を適用することができる。
