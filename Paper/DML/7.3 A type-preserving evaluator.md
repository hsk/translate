...
- 7.3 型温存評価
	...
	我々は現在、評価者が入力し保存型のオブジェクト·レベルであることをDMLの型システムでは、撮影、単純に型指定されたλ計算に基づいてオブジェクト言語のための評価を実施する。
	...
	我々は、実際にすでにプログラミング言語ATS（XI、2005）でこれを行っている。
