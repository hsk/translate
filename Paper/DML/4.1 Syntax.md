...
`P~ = P1, ..., Pn`の場合、インデックスコンテキスト `φ` は、`P1 ⊃ (...(Pn ⊃ τ )...)` の `a1 : s1, ..., an : sn; P~ ⊃ τ` があります。
...
値制限の形がλΠ、Σパットに課されていることに注意してください：それはeは式のΠ+（E）と⊃+（E）を形成するために、値であることが要求される。

値制限のこの形式は、一般的には非常にそのような第6節にλΠ、Σのパットに追加される参考文献（ライト、1995）、などの副作用の特徴の治療を簡素化することができます。

私たちは、実際に少しだけΠ+（E）または⊃+（E）が形成されたときにその電子が（代わりに値の）値と同等の表現である必要とすることによって、セクション6.3の値制限のこのフォームを緩和する必要があります。

一般的に言えば、正式に後で定義される値と同等の表現は、値に動作的に等価である表現を指す。
...
直感的には、型τの値にガードさタイプP⊃τの値を有効にするために、我々は命題Pを確立する必要があります。アサートタイプのP∧τの値が生成される場合、我々は命題Pが保持していると仮定することができます。
...
このタイプは、ブール式が成立するかどうか、実行時にチェックする機能に割り当てることができる。
ブール式が保持するために失敗した場合には、例外のいくつかのフォームを高くすることです。
...
実際には、我々はまた、サブセットソートの概念を持っている。
私たちは、サブセットをオーバー範囲にSを使用
...
私たちは、φを使用します。 P〜| - I：{：S | P1、...、Pnが}、φを意味する。 P〜| - I：sが導き出せるとφ。 P〜| - パイ[7私は→]保持のためのI = 1、...、N。
サブセットのソートSを考えると、私たちはπAを書きます。■。 πAためのτ：S。 τsがSである場合、またはπAのために：S1。 {：| P S1} sがある場合、P⊃τ。
同様に、私たちはΣAを書きます。■。 ΣAためのτ：S。 τsがSである場合、またはΣAのために：S1。 {：| P S1} sがある場合、P∧τ。
たとえば、私たちはΠa1を書く：NATを。 INT（A1）→Σa2：NAT。次のタイプのためにINT（A2）：
