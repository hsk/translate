...
言い換えれば、定数コンストラクタ cc が与えられ、我々は cc に割り当てられた c-type に基づいて、この特定のccのルール（pat-const）を形成することができます。
...
次の補題は、補題2.1に平行です。
我々は、この補題の最初の2つの文を証明する際に、ここで関係する制約関係が正則でという仮定を利用する必要があります。
...
- Lemma 4.3 (Thinning)
	...
	1. `φ` で宣言されていないすべてのインデックス変数 `a` のために、我々は `height(D) = height(D')` のような導出 `D' :: φ; P~, P; Γ |- e : τ` があります。
	2. `φ |- P : bool` が誘導可能であるように、すべてのPのために、我々は `height(D) = height(D')` その導出 `D' :: φ; P~, P; Γ |- e : τ` はそのようなが成り立ちます。
	3. `Γ` にと `φ |- τ0[type]` が導出であるようにτ0宣言されていないすべての変数XFのために、我々は `height(D) = height(D')` そのような導出 `D':: φ; P~ ; Γ, xf : τ0 |- e : τ` を持っている。
	...
	次の補題は `λΠ,Σ pat` もバッチリで、閉じた値の型と定理4.12、λΠための進歩定理、Σのパットの証明で必要とされる値の形の間に密接な関係を示している。
