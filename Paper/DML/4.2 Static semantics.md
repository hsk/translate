...
我々は、図13にリストされているタイプとコンテキストを形成するためのルール、で始まる。
我々は、構文を使用する| - タイプ索引語I1が与えられたときに我々はタイプδ（I1、...、In）を構築することができることを示すために、δ（S1、...、SN）、...、種s1のでは、 ...、snは、それぞれ。
...
我々は、φを書く。固定型のインデックス言語L.における定期的な制約関係のために= P0| P〜
...
我々は、そのτを言うと両方がτ≤stpτ 'とτ'≤stpτ成り立つ場合に等しいτ0である。
...
我々は、4.6節≤stpよりはるかに強いと名前ダイナミックサブタイプ関係を与えられている別のサブタイプ関係≤dtp、で紹介している。
...
- Lemma 4.1
	...
	1.もしφ、φ0。 、φ0はその後、成り立つφ：Θ - | P〜|とφ導出=τ'をτである≤stp P〜[Θ]|=τ[Θ]≤stpτ'[Θ]も導出可能。
...
我々は現在、図16と図17の式については、図15のパターンとその後型付け規則のためのタイピング規則を提示する。
...
ここで値の制限の存在に注目してください。
- タイプP⊃τの式eを考えると、型付け規則（TY-⊃-ELIM）は命題Pが成り立つ場合の式⊃-（e）が形成することができると述べている。
直感的には、保護された式はガードが排出することができる場合にのみ有効です。
- eは型τを割り当てることができ、Pは、その後型付け規則（TY-∧-イントロ）∧（E）をアサートタイプP∧τを割り当てを成り立つ場合。
...
- Lemma 4.6
	...
	- 証明
		...
		Dで最後に適用されたルールがある場合（TY-ラム）を処理するとき、我々は補題4.5を利用するため、（TY-VAR'）で（TY-VAR）を交換する必要性を参照してください。 □
